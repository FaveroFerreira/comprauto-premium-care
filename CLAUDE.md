...
Tauri 2.0 desktop application for automotive workshop management (Rech Performance). React 19 + TypeScript frontend with Rust backend. UI is in Portuguese (pt-BR).
...
- **State**: Zustand (auth, persisted to localStorage) + TanStack React Query (server state, 5-min cache)
- **UI**: Radix UI components wrapped in `components/ui/`, Tailwind CSS, Lucide icons
- **Path alias**: `@/` maps to `src/`

### Backend (`src-tauri/`)
- **Tauri 2.0** with SQLite (rusqlite), Argon2 password hashing
- **Command modules**: `commands/auth.rs`, `commands/vehicles.rs`, `commands/parts.rs`, `commands/service_orders.rs`, `commands/quotes.rs`, `commands/expenses.rs`
